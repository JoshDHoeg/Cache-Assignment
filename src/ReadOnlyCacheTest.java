...
public class ReadOnlyCacheTest
{
	...
	public final static int MEMORY_SIZE = 512*BYTES_PER_KB;
	public final static Random s_rand = new Random(0);
	...
	private ReadOnlyCache newInstance(int blockCount, int bytesPerBlock, int associativity)
	{
		...
		return new /*Solution*/Cache(m_memory, blockCount, bytesPerBlock, associativity);
	}
	...
	private void run()
	{
		...
		int nPassedAssertions = (m_nAssertions-m_nFailedAssertions);
		System.err.println(String.format("OVERALL SCORE: PASSED %d/%d = %f%%",
				nPassedAssertions, m_nAssertions,
				(100.0*nPassedAssertions)/((double)m_nAssertions)));
	}
	...
	{	final String THIS_METHOD = new Object(){}.getClass().getEnclosingMethod().getName();
		...
		for(int n=0; n<nBytesPerBlock; n++){
		    ...
		    assertEquals(THIS_METHOD,  String.format("Mem[%d] value", i), m_memory.peek(i), cache.load(i));
	...
	private void testDirectMapped(int nBlocks, int nBytesPerBlock)
	{
		...
		for(int t=0; t<3; t++){
			for(int i=0; i<nElements; i++){
				...
				assertEquals(THIS_METHOD, "sequential value test", m_memory.peek(address), cache.load(address));
			}
		}
